...
> **Prerequisite:** Run `ai-career-toolkit init` to set up your config files. The prompts below assume `config/settings.yaml` and `~/.ai-career-toolkit/role-thesis.md` are personalized. If they aren't, run `ai-career-toolkit personalize` first.
...
| Prepare for a behavioral interview | `star-story` → `mock-interview-loop` | `interview-prep-coach` |
| Practice a full interview loop | `mock-interview-loop` | `interview-prep-coach` |
| Write a STAR story from rough notes | `star-story` | — |
...
| Review/polish a draft (non-technical) | `content-review` | `wordsmith-editor` |
| Review technical writing | `tech-content-review` | `wordsmith-editor` |
...
The toolkit includes four agent personas. Your AI platform can delegate to them automatically, or you can invoke them by name.
...
**Handoffs:** Delegates to `research-guru` for evidence, `wordsmith-editor` for rewrites, `interview-prep-coach` for prep.

### interview-prep-coach
**When:** You're preparing for interviews — mock practice, story selection, readiness assessment.
**Returns:** Scored mock rounds with specific feedback, suggested improvements.
**Handoffs:** Uses `star-story` and `mock-interview-loop` skills.
...
**Handoffs:** Uses `content-review`, `tech-content-review`, `in-my-voice`, `social-content` skills.
...
> "Use the `star-story` skill to convert these rough notes into a STAR narrative."
...
|------|-------------|-------------|
| `config/settings.yaml` | Targeting criteria (role, level, domains, geo, exclusions) | `ai-career-toolkit personalize` or edit directly |
